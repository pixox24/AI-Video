...
1. 钩子：ms 中远景全身（角色约占画面高度一半，上三分留纵深），平视，三分。禁止 ws 开场。  
2. 建立：ws 同空间  
3. 细节：insert 或 ecu  
4. 对照才换主体  
...
`compileImagePrompt` 仍输出一条 `Use / Scene / Subject / Details / Constraints`，但按「全片不变量 / 本镜导演指令」分层写入：

| 段 | 全片不变量 | 本镜变量 |
|---|---|---|
| Use | 比例、体裁 | `frame i of n`；仅第 0 镜加 `hook` |
| Scene | 空间锁（只写一次） | 机位句 + 主体占画面高度 / 留白。**机位只出现在 Scene** |
| Subject | CHARACTER LOCK / OBJECT LOCK | 本镜未完成事件；禁止复述场景、禁止口播原句 |
| Details | 介质、色板、光、材质 | **不含镜头焦段**；hook 镜补主次光 |
| Constraints | 无字 / 参考图职责 / 不换装换房 | 本镜不要换滤镜 |

例：`中远景全身，平视，三分，钩子，主体约占画面高度一半，上三分留纵深`
...
故事 9:16 第一镜：中远景全身而不是纯全景；Subject 有角色锚点；action 有尚未完成的可见前兆。  
第二镜与第一镜景别不同（科普优先插入/机制；故事优先 ws 建立）。  
最后一镜能认成开场构图家族。  
对照句两图对切，不在中间断开口播。
