...
写作风格是题材（genre）与节奏（pace）之外的第三条轴，回答"怎么说话"。三档内置档案：analyst 冷静拆解、narrator 故事叙事、pundit 犀利观点；可在设置页粘贴 2–3 篇自己的历史文案反推自定义档案，确认锁定后只能另存。选中后风格块只**追加**到分段草稿与修订提示词，不改变承诺、证据与时长预算；未选中时不产生任何风格相关请求字段、提示词文本或评估条款，行为与选中前逐字节一致。

风格检查有两层：`src-server/style/lint.ts` 的 `styleLint` 是纯函数，做禁用表达、抽象占位词与档案声明的句长阈值检查，不调用模型；质量评估器按选中的档案逐段给出第五类 `style` Issue。风格 Issue 的 severity 上限为 medium，永不阻断导出，修复走既有两轮闭环，且锁定章节不会被回改。内置档案的三段范例/反例目前是通用骨架（`provisional: true`），尚未用真实优质文案归纳，待用反推端点替换。
