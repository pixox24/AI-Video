...
const revisionSchema = z.object({ narration: z.string().trim().min(1), usedEvidenceIds: z.array(z.string()),
  beats: z.array(z.object({ id: z.string().optional(), order: z.number().optional(), function: z.enum(['hook','setup','turn','proof','reveal','cta']),
...
export async function executeSectionRevision(input: RevisionInput): Promise<RevisionResult> {
  ...
  try {
    ...
    if (input.strict && !splitCoversSource(toLooseList(parsed.beats).map(b => String(b.narration || '')), narration)) {
      return { status: 503, sections, code: 'draft_contract_failed', error: '修订节拍未覆盖本章口播。' };
    }
    const section: ScriptSection = { ...current, narration, status: 'ready',
      beats: Array.isArray(parsed.beats) && parsed.beats.length ? toLooseList(parsed.beats).map((beat, index) => ({
        ...
        function: (beat.function || current.beats?.[index]?.function || 'setup') as ScriptSection['beats'][number]['function'],
